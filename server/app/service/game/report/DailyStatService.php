...
class DailyStatService
{
    ...
    public function rebuild(string $date): array
    {
        ...
        $union = implode(' UNION ALL ', array_map(fn ($table) => "SELECT {$fields} FROM `{$table}` WHERE merchant_id > 0 AND (business_date = ? OR platform_date = ?) AND delete_time IS NULL", $tables));
