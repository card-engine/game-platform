...
class ApiController extends OpenController
{
    ...
    public function games(Request $request): Response
    {
        (new MgsAuthService())->browserUser($request);
        $currency = strtoupper((string) $request->input('currency_code', (new MgsConfigService())->get('default_currency', config('mgs.default_currency', 'USD'))));
        $games = Game::with('brand:id,platform_brand_code,name,names')->when($request->input('game_type'), fn ($q, $v) => $v === 'other' ? $q->whereNotIn('game_type', ['slot', 'fish', 'table', 'poker', 'sport']) : $q->where('game_type', $v))->when($request->input('brand_code'), fn ($q, $v) => $q->whereHas('brand', fn ($b) => $b->where('platform_brand_code', $v)))->when($request->input('keyword'), fn ($q, $v) => $q->where('name', 'like', "%{$v}%"))->whereJsonContains('currency_codes', $currency)->orderBy('sort')->orderByDesc('id')->paginate(min(500, max(1, (int) $request->input('limit', 50))), ['*'], 'page', max(1, (int) $request->input('page', 1)));
