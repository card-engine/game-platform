...
class ApiController extends OpenController
{
    ...
    public function games(Request $request): Response
    {
        (new MgsAuthService())->browserUser($request);
        $currency = strtoupper((string) $request->input('currency_code', (new MgsConfigService())->get('default_currency', config('mgs.default_currency', 'USD'))));
        $games = Game::with('brand:id,code,name,names')->when($request->input('game_type'), fn ($q, $v) => $q->where('game_type', $v))->when($request->input('brand_code'), fn ($q, $v) => $q->whereHas('brand', fn ($b) => $b->where('code', $v)))->when($request->input('keyword'), fn ($q, $v) => $q->where('name', 'like', "%{$v}%"))->whereJsonContains('currency_codes', $currency)->orderBy('sort')->orderByDesc('id')->paginate(min(500, max(1, (int) $request->input('limit', 50))), ['*'], 'page', max(1, (int) $request->input('page', 1)));
        return $this->success(['list' => collect($games->items())->map(fn ($game) => ['mgs_game_id' => $game->id, 'name' => $game->name, 'names' => $game->names, 'icon_url' => $game->icon_url, 'mgs_brand_id' => $game->brand_id, 'brand_code' => $game->brand?->code, 'brand_name' => $game->brand?->name, 'game_type' => $game->game_type, 'currency_codes' => $game->currency_codes, 'is_hot' => $game->is_hot, 'is_new' => $game->is_new, 'status' => $this->available($game) ? 1 : 0, 'unavailable_reason' => $game->unavailable_reason])->all(), 'total' => $games->total(), 'page' => $games->currentPage(), 'limit' => $games->perPage()]);
    }
