...
MGS 是游戏平台的正常 B 端客户，同时是面向 C 端普通用户的独立产品。两套业务代码在同一个项目中部署，只是为了复用基础设施；业务数据、Model、Logic、Service、统计和后台菜单必须完全隔离。
...
```text
id, unique_id, nickname, language, status, last_login_time,
create_time, update_time, delete_time
```
