...
- 打开充值只通过 `/api/recharges/current` 恢复本用户、当前币种的未过期 `pending` 或 `review` 订单，不再从本地订单号恢复历史单。`paid/expired/closed` 进入新建表单；历史详情在充值记录中查看。当前弹层内仍可显示刚到账的结果并刷新余额。
...
- 发请求前保存编号与到账币种、金额、支付币种；同一次提交的网络超时重试复用编号，提交中禁用重复点击。重新打开、刷新充值选项或点击新建时，先查询服务端当前订单，再清理本用户、本币种的旧订单指针和请求编号。存在可复用订单则继续付款或核验，否则展示新建表单，用户点击创建后才生成新编号、提交订单。查询失败不清理请求号，也不允许新建。后端仍按旧编号返回原订单，包括已结束订单；同编号换参数则拒绝。
