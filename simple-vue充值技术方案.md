...
**断点字段：** `start_block_number`、`next_block_number`、`last_block_hash`、`last_success_time`、`recovery_required`。实时跳过缺口后，`last_block_hash` 只表示新起点的前一块，不代表整段历史已扫完；启动时清空成功心跳，处理成功再更新，历史完整性同时看未完成缺口和恢复标记。

**补扫字段：** `from`、`to`、`next`、`last_hash`、`end_hash`、`attempts`、`retry_time`、`error`。任务 ID 随机生成；Redis时间使用 UTC Unix 秒。任务只存范围、进度和错误，不存区块正文。

### 5.5 初始化、丢失恢复与下单门禁

- 正式首次启用前，由管理员确认这是首次收款并初始化 Redis 起点；运行进程不通过 MySQL 猜测是否首次启动。
- Redis 断点缺失或损坏时，仍可从最新固化块建立实时扫描，但必须同时设置永久的 `recovery_required=1`，暂停新下单。管理员确认历史恢复范围并登记补扫，补齐且核验后才解除恢复标记；“实时已追到最新”不能自动解除。
- 恢复起点必须覆盖未确认扫描完成的区间。没有可信备份时，至少覆盖最早充值订单时间之前的区块；不能拿“最近一笔收款高度”当成完整扫描断点，也不能只补扫未支付订单。
- 断点被人为删除、Redis 故障切换或从备份恢复后，清除健康标记并核对断点和任务是否来自同一恢复点。无法证明没有缺口时按丢失恢复处理。
- 充值开关开启仍不足以接单：断点有效、`recovery_required=0`、心跳新鲜、固化头及实时扫描延迟正常，且没有未完成补扫任务，才允许新下单。补扫期间已固化收款照常处理，旧订单查询不受影响。
...
地址、合约、节点未配置，Redis 断点/健康状态缺失、恢复标记未解除、心跳超过 2 分钟、固化头长时间不更新、实时扫描落后固化头超过 2 分钟，或存在未完成补扫任务时，暂停新下单。充值开关关闭只停止新订单，不停止旧收款的扫描和处理。
...
| Redis | 断点重启、健康标记过期、持久化回退、断点丢失仍实时扫描但禁止新单、锁过期后旧进程不能推进 |
...
上线前配置收款地址、节点及只读 API Key，确认 Redis 持久化，初始化断点并补齐历史缺口。健康检查通过后再开放充值。用受控钱包分别验证 USDT/TRX 小额充值，核对“链上收款 → 订单 → 余额 → 流水”，重复扫描和投递后余额不变。
