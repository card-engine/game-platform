...
系统保留 `merchant_id=0` 作为后台试玩标识，但不在 `mg_merchants` 创建记录。数据库初始化固定创建 `mg_users.id=1` 的系统玩家，其商户玩家标识取自 `.env`。回调地址、密钥、币种、固定试玩余额、语言、时区和返回地址同样只从 `.env` 读取；自营钱包通过签名接口返回固定余额，不进入商户列表、企业权限范围、顶部商户选择或对接文档。

试玩仅开放给 SaiAdmin 超级管理员和游戏超管。游戏资源页直接使用内部游戏 ID 和上游资源字段启动，因此未归并品牌也可试玩；该链路不检查商户授权、额度和计费。试玩产生的玩家、注单和流水允许使用 `merchant_id=0` 便于排查，但日报、趋势、GGR、月费和平台汇总的源查询必须排除该标识。
...
- 超级管理员和游戏超管可点击币种标签试玩，系统固定使用 `.env` 中的自营参数，不依赖顶部当前商户。
