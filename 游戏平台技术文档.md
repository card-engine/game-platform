...
| 身份 | 主要能力 |
| --- | --- |
| 超级管理员 | 创建企业和商户、配置计费、调整额度、维护 MG 游戏平台状态、查看全部数据 |
| 企业负责人 | 管理本企业子账号、分配商户范围、维护回调参数和商户单款覆盖 |
...
创建商户时可以选择另一商户作为来源执行“复制覆盖配置”，只复制 `mg_merchant_games` 的单款状态、特殊费率和 RTP。
...
MGS 是游戏平台的正常 B 端客户，同时是面向 C 端普通用户的独立产品。它不属于游戏平台的企业权限或游戏平台统计口径，不写入游戏平台 `mg_*` 业务表。MGS 的完整数据域、路由、流程图和时序图见《自营平台技术文档.md》。
...
`mg_games.brand_id` 始终指向 `mg_game_brands.id`，因此每款游戏保留真实游戏平台和上游品牌来源。商户默认获得完整目录；统一品牌只用于展示、筛选和归并，不参与商户授权。

`mg_games.game_code` 使用 `统一品牌code_MG游戏标识`，例如 `jili_60972...`。统一品牌 code 强制小写，编码不包含 WxGame、TADA 等上游平台标识。
...
未归并品牌及其游戏仍显示在品牌资源和游戏资源中，列表会返回 `brand_mapping_status`，由下游决定是否展示或开放。
...
| 商户游戏覆盖 | 设置单款关闭、费率或 RTP 覆盖 |

`mg_merchant_games` 只保存例外：单独关闭或覆盖费率/RTP。没有记录表示默认允许。最终可用性由上游状态、MG 平台状态、游戏平台总开关、商户覆盖和币种共同计算。
...
| `mg_merchant_games` | `merchant_id + game_id`，单款状态、特殊费率和默认 RTP；无记录默认允许 |
...
进游请求的 `nickname` 为可选字段，首次进游时写入玩家资料。可选 `ip` 传玩家 IPv4/IPv6 地址并参与签名，用于最近进游 IP；未传时保留已有值，新玩家留空。调用服务器 IP 只用于来源校验，后台直接试玩仍使用操作者的请求 IP：
...
- 自营平台的游戏、用户和 C 端进游不在本页面管理，统一按《自营平台技术文档.md》使用 MGS 独立后台和 `/api/*` 接口。
